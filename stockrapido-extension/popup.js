...
async function render() {
  ...
  if (stored.syncStatus) { progress.textContent = stored.syncStatus.message; syncButton.disabled = stored.syncStatus.status === 'running'; }
}
...
syncButton.addEventListener('click', async () => { error(''); syncButton.disabled = true; progress.textContent = 'Iniciando…'; await chrome.runtime.sendMessage({ type: 'START_SYNC' }); });
...
chrome.runtime.onMessage.addListener((message) => { if (message?.type !== 'SYNC_PROGRESS') return; progress.textContent = message.payload.message; syncButton.disabled = message.payload.status === 'running'; });
