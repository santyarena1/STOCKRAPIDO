...
export class FiscalService {
  ...
  async saveConfig(businessId: string, dto: SaveConfig) {
    ...
    if (cuit.length !== 11) throw new BadRequestException('El CUIT debe tener 11 dÃ­gitos.');
    if (!Number.isInteger(dto.pointOfSale) || dto.pointOfSale < 1 || dto.pointOfSale > 99999) throw new BadRequestException('Punto de venta invÃ¡lido.');
    if (!['homologation', 'production'].includes(dto.environment)) throw new BadRequestException('Ambiente invÃ¡lido.');
    ...
    if (dto.certificate?.trim()) {
      ...
      } catch { throw new BadRequestException('El certificado no es un PEM/X.509 vÃ¡lido.'); }
    }
    if (dto.privateKey?.trim()) {
      ...
      catch { throw new BadRequestException('La clave privada no es un PEM vÃ¡lido.'); }
    }
  ...
  private validatePair(certPem: string, keyPem: string) {
    ...
    } catch { throw new BadRequestException('El certificado y la clave privada no corresponden entre sÃ­.'); }
  }

  private async credentials(businessId: string) {
    const config = await this.prisma.fiscalConfig.findUnique({ where: { businessId } });
    if (!config) throw new BadRequestException('ConfigurÃ¡ ARCA antes de emitir Factura C.');
  ...
  private async auth(businessId: string) {
    ...
    if (!response.ok) throw new Error(`WSAA respondiÃ³ HTTP ${response.status}.`);
    const outer = this.parser.parse(text);
    const returned = outer?.Envelope?.Body?.loginCmsResponse?.loginCmsReturn;
    if (!returned) throw new Error(this.soapError(outer) || 'WSAA no devolviÃ³ credenciales.');
    const login = this.parser.parse(returned)?.loginTicketResponse;
    if (!login?.credentials?.token || !login?.credentials?.sign) throw new Error('Respuesta de autenticaciÃ³n de ARCA invÃ¡lida.');
  ...
  private async wsfe(environment: string, action: string, body: string) {
    ...
    if (!response.ok || this.soapError(parsed)) throw new Error(this.soapError(parsed) || `WSFE respondiÃ³ HTTP ${response.status}.`);
  ...
  async testConnection(businessId: string) {
    try {
      ...
      return { ok: true, environment: config.environment, pointOfSale: config.pointOfSale, lastAuthorized: Number(result?.CbteNro || 0), message: 'ConexiÃ³n y autorizaciÃ³n con ARCA correctas.' };
  ...
  private collectErrors(result: any) {
    ...
    if (!raw) return '';
    return (Array.isArray(raw) ? raw : [raw]).map((x: any) => `${x.Code || x.Code === 0 ? `[${x.Code}] ` : ''}${x.Msg || ''}`).join(' Â· ');
  }
  ...
  async issueFacturaC(businessId: string, saleId: string) {
    ...
    try {
      const { config, token, sign } = await this.auth(businessId);
      if (!config.enabled) throw new Error('La facturaciÃ³n ARCA estÃ¡ desactivada en ConfiguraciÃ³n.');
      ...
      const errors = this.collectErrors(result);
      if (result?.FeCabResp?.Resultado !== 'A' || detail?.Resultado !== 'A' || !detail?.CAE) throw new Error(errors || 'ARCA rechazÃ³ el comprobante sin detalle.');
