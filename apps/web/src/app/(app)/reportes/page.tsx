...
function Table({ headers, children, empty }: { headers: string[]; children: ReactNode; empty: boolean }) {
  return <div className="overflow-x-auto rounded-xl border border-hair-soft"><table className="w-full text-sm"><thead className="bg-raised text-left text-xs uppercase tracking-wide text-fg-faint"><tr>{headers.map((header) => <th key={header} className="whitespace-nowrap px-3 py-2.5">{header}</th>)}</tr></thead><tbody className="divide-y divide-hair-soft">{children}{empty && <tr><td colSpan={headers.length}><Empty /></td></tr>}</tbody></table></div>;
}
