    ...
    <Container className="max-w-4xl space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-hair-soft bg-surface p-4 sm:p-6">
        <div className="grid gap-5 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center">
          ...
          <div><div className="flex flex-wrap gap-2">{product.incomplete ? <span className="rounded-md border border-warn/30 bg-[var(--warn-soft)] px-2 py-1 text-xs text-warn">Incompleto</span> : <span className="rounded-md border border-ok/30 bg-[var(--ok-soft)] px-2 py-1 text-xs text-ok">Completo</span>}<span className={`rounded-md border px-2 py-1 text-xs ${product.isActive === false ? 'border-crit/30 bg-[var(--crit-soft)] text-crit' : 'border-ok/30 bg-[var(--ok-soft)] text-ok'}`}>{product.isActive === false ? 'Inactivo' : 'Activo'}</span><DeliveryProductReadinessBadges providers={deliveryProviders} /></div><h1 className="mt-3 text-2xl font-bold text-fg">{product.name}</h1><p className="mt-1 text-sm text-fg-muted">{displayValue(product.brand)} · {displayValue(product.category?.name)} · {displayValue(product.subcategory)}</p><div className="mt-4 grid grid-cols-2 gap-3 sm:max-w-xl"><InfoCell label="Precio" value={moneyValue(product.price)} mono /><InfoCell label="Stock" value={`${product.stock} unidades`} mono /></div></div>
        </div>
      </section>
