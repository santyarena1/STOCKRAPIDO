  ...
  return <Container className="max-w-6xl space-y-6">
    ...
    {connection && <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <section className="rounded-xl border border-hair-soft bg-surface p-5"><div className="mb-5 flex gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-raised2 font-mono text-fg-muted">1</span><div><h2 className="font-semibold text-fg">Sincronizar catálogo</h2><p className="text-sm text-fg-faint">{connection._count?.items ?? items.length} ítems disponibles</p></div></div>{connection.provider === 'tokin' ? <div className="space-y-3"><p className="text-sm leading-5 text-fg-muted">Tokin se sincroniza desde tu navegador con la extensión (trae catálogo, costos, cuenta y pedidos).</p><Link href="/config/proveedores" className="btn-brand flex w-full items-center justify-center rounded-xl px-4 py-3 text-sm font-semibold">Abrir extensión / Descargar</Link></div> : <button type="button" onClick={() => run('run')} disabled={!!busy || connection.provider !== 'mondelez'} className="w-full rounded-xl border border-hair bg-raised px-4 py-3 text-sm font-semibold text-fg hover:bg-raised2 disabled:opacity-50">{connection.provider !== 'mondelez' ? 'Usá el runner local' : busy === 'run' ? 'Sincronizando…' : 'Sync catálogo (servidor)'}</button>}</section>
        <section className="rounded-xl border border-hair-soft bg-surface p-5"><div className="mb-5 flex gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-raised2 font-mono text-fg-muted">2</span><div><h2 className="font-semibold text-fg">Traer precio B2B</h2><p className="text-sm text-fg-faint"><span className="font-mono text-ok">{withCost}</span> de {connection._count?.items ?? items.length} con precio</p></div></div><p className="text-sm text-fg-muted">{connection.provider === 'tokin' ? 'La extensión ya trae el costo B2B real con tu sesión de Tokin.' : 'El runner local actualiza los costos reales del proveedor.'}</p></section>
      ...
      {connection.provider === 'tokin' ? <section className="rounded-xl border border-hair-soft border-l-4 bg-surface p-4 text-sm text-fg-muted"><p className="font-semibold text-fg">Extensión de navegador</p><p className="mt-1">Descargá la extensión en Configuración → Proveedores, logueate en <span className="font-mono text-xs text-fg">tokintienda.com.ar</span> y usá “Sincronizar Tokin”.</p><Link href="/config/proveedores" className="mt-3 inline-flex text-sm font-semibold text-brand hover:underline">Ir a Configuración → Proveedores</Link></section> : <section className="rounded-xl border border-hair-soft border-l-4 bg-surface p-4 text-sm text-fg-muted"><p className="font-semibold text-fg">Runner local</p><p className="mt-1">{meta?.runnerNote}</p><p className="mt-2 text-xs text-fg-faint"><code>SR_API={apiBase || 'https://stockrapido-api.vercel.app'}</code> · ejecutá <code>python {meta?.runnerFile ?? `${connection.provider}_sync_runner.py`}</code>.</p></section>}
    </>}
  </Container>;
}
