  ...
  return (
    <>
      ...
      <Link
        ...
        className="fixed bottom-4 right-4 z-40 flex items-center gap-2 rounded-full border border-hair bg-surface px-3.5 py-2 text-sm font-medium text-fg shadow-lg transition hover:bg-raised"
      >
        <span aria-hidden>🔔</span>
        <span>Novedades</span>
        <span className="rounded-md bg-[color:var(--brand-accent)] px-1.5 py-0.5 font-mono text-[11px] font-bold text-white">
          {CURRENT_VERSION}
        </span>
      </Link>

      {/* Popup de novedades al entrar */}
      {showPopup && latest && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4" onClick={dismiss}>
          ...
          >
            <div className="flex items-center justify-between gap-3 border-b border-hair-soft px-5 py-4">
              ...
              <button type="button" onClick={dismiss} className="rounded-lg px-2 py-1 text-fg-muted hover:bg-raised">✕</button>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 border-t border-hair-soft px-5 py-3">
              <button type="button" onClick={openFull} className="rounded-lg border border-hair bg-surface px-4 py-2 text-sm font-medium text-fg hover:bg-raised">
                Ver más
              </button>
